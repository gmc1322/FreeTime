...
class KarmaTree
{
  public:
    class Responses
    {
      public:
        ...
        {
          public:
            explicit constexpr ConstIter( KarmaTreeSpace::TreeText::ResponseContainer::const_iterator &&ConstIter ) noexcept;

          public:
            friend bool operator!=( const KarmaTreeSpace::TreeText::ResponseContainer::const_iterator &Lhs, const ConstIter &Rhs ) noexcept;

          private:
            const KarmaTreeSpace::TreeText::ResponseContainer::const_iterator IterIndex;
        };
        ...
        class Iter
        {
          ...
          public:
            explicit Iter( KarmaTreeSpace::TreeText::ResponseContainer::const_iterator &&Iter ) noexcept;
          ...
          public:
            const KarmaTreeSpace::TreeText::ResponseType GetCurrentResponse() const noexcept;

          private:
          KarmaTreeSpace::TreeText::ResponseContainer::const_iterator IterIndex;
        };

      public:
        explicit constexpr Responses( const KarmaTreeSpace::TreeText::ResponseContainer &Response ) noexcept;
      ...
      private:
        const KarmaTreeSpace::TreeText::ResponseContainer &PlayerResponses;
    };
  ...
  private:
    const KarmaTreeSpace::TreeText Head;

    mutable const KarmaTreeSpace::TreeText *CurrentText = &Head;                          //!< Mutable since only the ptr changes, not the text.
    mutable KarmaTreeSpace::TreeText::ResponseContainer CurrentOptions;                   //!< Mutable since no text change.
    mutable std::stack< KarmaTreeSpace::TreeText::ResponseContainer > CurrentOptionStack; //!< Mutable since no text change.

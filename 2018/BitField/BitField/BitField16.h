...
template<>
class BitField< ushort_BitCount >
{
  public:
  ...
  public:
    static constexpr uchar BitCount = ushort_BitCount;
  ...
  public:
    ...
    template< MaxBits Count >
    const BitField< ushort_BitCount + Count > operator+( const BitField< Count > &Rhs  ) const noexcept;
    template< MaxBits Count >
    const BitField< ushort_BitCount + Count > operator+(       BitField< Count > &&Rhs ) const noexcept;
  ...
  public:
     ...
     */
    const std::vector< ushort > Number( std::initializer_list< std::pair< size_t, size_t > > &&Intervals ) const ExceptDebug;

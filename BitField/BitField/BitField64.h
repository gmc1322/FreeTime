...
template<>
class BitField< ullong_BitCount >
{
  public:
  ...
  public:
    static constexpr uchar BitCount = ullong_BitCount;
  ...
  public:
    ...
    template< MaxBits Count >
    const BitField< ullong_BitCount + Count > operator+( const BitField< Count > &Rhs  ) const noexcept;
    template< MaxBits Count >
    const BitField< ullong_BitCount + Count > operator+(       BitField< Count > &&Rhs ) const noexcept;
  ...
  public:
     ...
     *
     * \return std::vector< ullong > - The numbers from each range.
     *
     * The range is [Start,End].
     * \par
     * The intervals do not need to be connected.
     */
    const std::vector< ullong > Number( std::initializer_list< std::pair< size_t, size_t > > &&Intervals ) const ExceptDebug;

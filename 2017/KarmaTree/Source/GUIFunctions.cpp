...
{
  static KarmaTreeWindow_Trunk Trunk;
  ...
  KarmaTreeWindow_Base::RunAllWindows();
...
{
  static Button CreateKarmaTreeCode( ImColor( 1.0f, 0.0f, 0.0f ), "Create Code" );
  static Button SaveKarmaTreeFile( ImColor( 0.0f, 1.0f, 0.0f ), "Save Tree" );
  static Button LoadKarmaTreeFile( ImColor( 1.0f, 0.0f, 0.0f ), "Load Tree" );
  static Input KarmaTreeFileName( "File Name" );
  ...
  if( !KarmaTreeWindow_Base::GetIsFocused() )
  {
  ...
  KarmaTreeFileName.RunInput();

  ImGui::NewLine();

  // These are variables to keep the buttons rendered when in the IF statements!
  auto CreatedTree = CreateKarmaTreeCode.RunButton();
  /*auto SavedTree =*/ SaveKarmaTreeFile.RunButton();
  /*auto LoadedTree = */LoadKarmaTreeFile.RunButton();

  if( CreatedTree )
  {
    CreateConversation( KarmaTreeFileName.GetInput() );
  }/*
  else if( SavedTree )
  {
    SaveTree( KarmaTreeFileName.GetInput() );
  }
  else if( LoadedTree )
  {
    LoadTree( KarmaTreeFileName.GetInput(), nullptr );
  }*/

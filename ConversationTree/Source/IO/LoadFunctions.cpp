...
{
  public:
    LoadBase( ConversationTreeWindow_BasePtr Base ) noexcept { LoadNode( Base ); }

  private:
    static void LoadNode( ConversationTreeWindow_BasePtr Base ) noexcept
    {
...
void LoadTree( const char* FileName ) noexcept
{
  ...
  if( file.is_open() )
  {
    ...
    auto AllWindows = ConversationTreeWindow_Base::EditAllWindows();
    ...
    //LoadTrunk Trunk( static_cast< ConversationTreeWindow_NPCPtr >( AllWindows->at( 0 ) ) );
